...
module.exports = async(xinz, msg, smsg, blocked, _afk, welcome) => {
    try {
        ...
        const isAntiPromosi = isGroup ? antipromosi.includes(from) : false
       ...
       if (isButton && isCmd && !quotedMsg.fromMe) return
        ...
        // Anti promosi
        if (isGroup && isAntiPromosi && !isOwner && !isGroupAdmins && isBotGroupAdmins){
            if (chats.match(/(https:\/\/youtu.be)/gi)) {
                reply(`*「 ANTI PROMOSI DETECTOR 」*\n\nDi grup ini dilarang promosi,jadi maaf banget kamu akan dikick`)
                xinz.groupRemove(from, [sender])
            }
        }
        //Anti VO
        ...
        switch(command){
            ...
            case prefix+'menu': case prefix+'help':{
             ...
             case prefix+'allmenu':{
                    ...
                    xinz.sendList(from, `Selamat ${ucap}`, `Hai kak @${sender.split('@')[0]}, pilih Menu NotBot disini`, `Jangan lupa Donasi ya Kak`,`Pilih Disini`, list, msg, [sender])
                })
            }
                break
//------------------< PRAY Command >-------------------
...
//------------------< Text Marker >-------------------
            ...
            case prefix+'glitch': case prefix+'pornhub':
                ...
                limitAdd(sender, limit)
                break
            case prefix+'pubg': case prefix+'battlefield': 
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                if (args.length < 2) return reply(`Penggunaan ${command} text1|text2`)
                if (!q.includes("|")) return reply(`Penggunaan ${command} text1|text2`)
                reply(mess.wait)
                xinz.sendMessage(from, {url: `https://h4ck3rs404-api.herokuapp.com/api/textmaker/game?text=${q.split("|")[0]}&text2=${q.split("|")[1]}&theme=${command.split(prefix)[1]}&apikey=404Api` }, image, {quoted: msg}).catch(() => reply(mess.error.api))
                limitAdd(sender, limit)
                break
            ...
            case prefix+'glow':
                ...
                if (args.length < 2) return reply(`Penggunaan ${command} text`)
                reply(mess.wait)
                xinz.sendMessage(from, { url:`https://h4ck3rs404-api.herokuapp.com/api/textmaker/metallic?text=${q}&theme=glow&apikey=404Api` }, image, { quoted: msg }).catch(() => reply(mess.error.api))
                limitAdd(sender, limit)
                break
            case prefix+'angelwing': case prefix+'firewing': case prefix+'neondevil': case prefix+'papercut': case prefix+'sand': case prefix+'sand2':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                if (args.length < 2) return reply(`Penggunaan ${command} text`)
                reply(mess.wait)
                xinz.sendMessage(from, { url:`https://h4ck3rs404-api.herokuapp.com/api/ephoto/${command.split(prefix)[1]}?text=${q}&apikey=404Api` }, image, { quoted: msg }).catch(() => reply(mess.error.api))
                limitAdd(sender, limit)
                break
            case prefix+'thunder': case prefix+'1917':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                if (args.length < 2) return reply(`Penggunaan ${command} text`)
                reply(mess.wait)
                xinz.sendMessage(from, { url:`https://h4ck3rs404-api.herokuapp.com/api/textprome/${command.split(prefix)[1]}?text=${q}&apikey=404Api` }, image, { quoted: msg }).catch(() => reply(mess.error.api))
                limitAdd(sender, limit)
                break
            ...
            case prefix+'naruto':
                ...
                if (args.length < 2) return reply(`Penggunaan ${command} text`)
                reply(mess.wait)
                xinz.sendMessage(from, { url:`https://api.zeks.me/api/naruto?apikey=chika-key&text=${q}`}, image, { quoted: msg }).catch(() => reply(mess.error.api))
                limitAdd(sender, limit)
                break
//------------------< imagemaker >-------------------
...
//------------------< Media, Misc, and Fun >-------------------
            ...
            case prefix+'memeindo':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                reply(mess.wait)
                fetchJson(`https://h4ck3rs404-api.herokuapp.com/api/memeindo?apikey=404Api`)
                .then((res) =>{
                xinz.sendMessage(from, { url: res.result }, image, { quoted: msg }).catch(() => reply(mess.error.api))
                limitAdd(sender, limit)
                })
            ...
            case prefix+'darkjoke':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                reply(mess.wait)
                fetchJson(`https://h4ck3rs404-api.herokuapp.com/api/darkjoke?apikey=404Api`)
                .then((res) =>{
                xinz.sendMessage(from, { url: res.result }, image, { quoted: msg }).catch(() => reply(mess.error.api))
                limitAdd(sender, limit)
                })
				...
				if (isQuotedMsg && args.length < 2) {
                    ...
                    .then((res) => {
                        if (res.participants[0][quotedMsg.sender.split("@")[0] + '@c.us'].code === "403"){
                            ...
                            xinz.sendMessage(quotedMsg.sender, { groupName: groupName, groupJid: from, inviteCode: au.invite_code, inviteExpiration: au.invite_code_exp, caption: `Invited By NotBot`, jpegThumbnail: pepeqq }, groupInviteMessage)
					...
					.then((res) => {
                        if (res.participants[0][args[1] + '@c.us'].code === "403"){
                            ...
                            xinz.sendMessage(args[1] + '@s.whatsapp.net', { groupName: groupName, groupJid: from, inviteCode: au.invite_code, inviteExpiration: au.invite_code_exp, caption: `Invited By NotBot`, jpegThumbnail: pepeqq }, groupInviteMessage)
                            reply(`Mengirimkan groupInvite kepada nomor tersebut`)
...
//------------------< Enable / Disable >-------------------
            ...
            case prefix+'antilink':
                ...
                break
            case prefix+'antipromosi':
                if (!isGroup) return reply(mess.OnlyGrup)
                if (!isGroupAdmins && !isOwner) return reply(mess.GrupAdmin)
                if (!isBotGroupAdmins) return reply(mess.BotAdmin)
                if (args.length === 1) return reply(`Pilih enable atau disable`)
                if (args[1].toLowerCase() === 'enable'){
                    if (isAntiPromosi) return reply(`Udah aktif`)
                    antipromosi.push(from)
					fs.writeFileSync('./database/antipromosi.json', JSON.stringify(antiPromosi))
					reply('AntiPromosi grup aktif')
                } else if (args[1].toLowerCase() === 'disable'){
                    let anu = antipromosi.indexOf(from)
                    antipromosi.splice(anu, 1)
                    fs.writeFileSync('./database/antipromosi.json', JSON.stringify(antipromosi))
                    reply('antipromosi grup nonaktif')
                } else {
                    testqq(from, `antipromosi`)
                }
                break
